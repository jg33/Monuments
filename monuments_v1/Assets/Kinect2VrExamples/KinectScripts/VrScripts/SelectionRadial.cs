...
namespace VRStandardAssets.Utils
{
    ...
    public class SelectionRadial : MonoBehaviour
    {
		...
		public void SetRedialProgress(float fPercent)
		{
			if (m_IsSelectionRadialActive) 
			{
				// the bar is not filled.
				m_RadialFilled = false;

				// set the fill amount.
				m_Selection.fillAmount = fPercent;
			}
		}


		public void ResetRedialProgress()
		{
			if (m_IsSelectionRadialActive) 
			{
				// the bar is not filled.
